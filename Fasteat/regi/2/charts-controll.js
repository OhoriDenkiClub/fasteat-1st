...
.then(data => {
    ...
    for(let i=1;i<data.length; i++){
        ...
        if (!totalsByHour[hour]) {
            // {}の中を商品数に合わせて加減
            totalsByHour[hour] = { prod1: 0, prod2: 0, prod3: 0,total:0};
        }
        // 商品数に合わせて加減
        ...
        // 全ての商品を合計する
        totalsByHour[hour].total += item.prod1+item.prod2+item.prod3;
        // 商品数に合わせて加減
    ...
    Object.values(totalsByHour).forEach(hourData => {
        // ()の中を商品数に合わせて加減
    maxTotal = Math.max(maxTotal, hourData.prod1, hourData.prod2, hourData.prod3, hourData.total);
    });
    ...
    options: {
        plugins:{
            title:{
                display:true,
                text:"王道たこ焼き合計:"+totalProd1+'　明太マヨたこ焼き合計:'+totalProd2+'　チーズinたこ焼き合計:'+totalProd3
            }
        },
        scales: {
        x:{
