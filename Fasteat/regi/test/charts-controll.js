    ...
    .then(data => {
        ...
        for(let i=1;i<data.length; i++){
            ...
            if (!totalsByHour[hour]) {
                // {}の中を商品数に合わせて加減
                totalsByHour[hour] = { prod1: 0, prod2: 0, total:0};
            }
            // 商品数に合わせて加減
            totalsByHour[hour].prod1 += item.prod1;
            totalsByHour[hour].prod2 += item.prod2;
            // 全ての商品を合計する
            totalsByHour[hour].total += item.prod1+item.prod2;
            // 商品数に合わせて加減
        ...
        Object.values(totalsByHour).forEach(hourData => {
            // ()の中を商品数に合わせて加減
        maxTotal = Math.max(maxTotal, hourData.prod1, hourData.prod2, hourData.total);
        });
        ...
        options: {
            plugins:{
                title:{
                    display:true,
                    text:"商品1合計:"+totalProd1+'　商品2合計:'+totalProd2
                }
            },
            scales: {
            x:{
